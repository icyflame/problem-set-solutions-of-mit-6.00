...
class Hand(object):
    ...
    def update(self, word):
        ...
        for i in range(len(word)):
            if temp.get(word[i],-1) != -1:
                if(temp[word[i]]==1):
                    del temp[word[i]]  ##If this is the only letter and it is going to be used then we may as well delete it from the hand.
                else:
                    temp[word[i]] -= 1

        ##The hand has been updated.
    ...
    def containsLetters(self, letters):
        ...
        for i in copy1:
            ...
            if temp.get(i,-1) == -1:   ##This letter is not there in the hand
                return False

            else:
                ##If it is there then we have to reduce the frequency by one as it is being used once

                temp[i]-=1

                if(temp[i] == -1):  ##Letter has been used more than times it is there in the hand
                    return False

        ##If it comes out of the above loop then the word can be made out of the hand. Thus:
...
class Player(object):
    ...
    def __cmp__(self, other):
        ...
        ##If it is neither of the above two cases then definitely it is the
        ##case where the score of this player is less than other player
...
class ComputerPlayer(Player):
    ...
    def pickBestWord(self, wordlist):
        ...
        for i in word_list:
            if self.hand.containsLetters(i):                
                if getWordScore(i) > maxPts:
                    selected = i
                    maxPts = getWordScore(i)
