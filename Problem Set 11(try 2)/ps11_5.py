...
num_trials = 1  ##the number of trials to be run

##helper function for plot 1

def runSimluationOnOneRobotSeventyFivePercent(width,height):

    result = runSimulation(1,1,width,height,0.75,num_trials,Robot,False)
...
def runSimluationOnPlot2(noOfRobots):    

    result = runSimulation(noOfRobots,1,25,25,0.75,num_trials,Robot,False)
...
def runSimPlot4(min_coverage,no_robots):

    result = runSimulation(no_robots,1,25,25,min_coverage,num_trials,Robot,False)
...
avg1 = runSimulation(1,1,5,5,1,1,RandomWalkRobot,True)
