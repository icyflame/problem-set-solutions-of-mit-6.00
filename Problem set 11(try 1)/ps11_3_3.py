...
##
##        if hitAWall(cp,self.d,self.room.w,self.room.h):
##
##            print '------------------------------'
##            print
##            print
...
##
##            
##            while not self.room.isPositionInRoom(cp):
...
##        self.setRobotDirection(d)
        ...
        np = cp.getNewPosition(self.d,self.speed)

        if not self.room.isPositionInRoom(np):
            while self.room.isPositionInRoom(np):
                print '--------------------------------------------'
                d = random.choice(range(360))
                print 'random direction is:',d
                
                np = cp.getNewPosition(s,self.speed)

                print 'new position is:',np.x,',',np.y

##        while (correct(cp) == correct(np)):
##            np = np.getNewPosition(self.d,self.speed)

##        if hitAWall(nep,self.d,self.room.w,self.room.h):
##            d = self.d
##            while not hitAWall(nep,d,self.room.w,self.room.h):
##                d = random.choice(range(360))
##
##                np = cp.getNewPosition(d,self.speed)
##
##                nep = correct(np)
