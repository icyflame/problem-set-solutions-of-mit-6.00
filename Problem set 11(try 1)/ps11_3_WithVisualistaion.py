...
class Robot(BaseRobot):
    ...
    def updatePositionAndClean(self):
        ...
        newPos = currentPos.getNewPosition(self.d,self.speed)

        newPos = correct(newPos)

        hitAWall = [Position(0,0),Position(0, self.h - 1), \
                    Position(self.w - 1,0),Position(self.w - 1,swlf.h - 1)]

        if newPos in hitAWall:
            pass

        else:
            while correct(newPos) == correct(currentPos):
                newPos = newPos.getNewPosition(self.d,self.speed)
        ...
        if self.room.isTileCleaned(newPos.x ,newPos.y):
            print 'tile at ',newPos.x,',',newPos.y,' was cleaned'

        else:
            print 'tile at ',newPos.x,',',newPos.y,' has been cleaned'
